...
(function() {
    ...
    this.screenCreditApplication = function(application) {
        var self = this;
        return new Promise(function(resolve, reject) {
            self.authenticate().then(function() {
                var options = self.getOptions("screen-unsecured-credit-application");
                options.pathname = options.pathname.replace('{applicationId}', application.applicationId)
                options.uri = options.uri.replace('{applicationId}', application.applicationId)
                options.body = {
                    controlFlowId : application.controlFlowId
                }
                self.handleRequest(options).then(function(application){
                    resolve(application);
                })
            });
        });
    ...
    this.requestCreditApplicationDecision = function(application) {
        var self = this;
        return new Promise(function(resolve, reject) {
            self.authenticate().then(function() {
                var options = self.getOptions("request-credit-application-decision");
                options.pathname = options.pathname.replace('{applicationId}', application.applicationId)
                options.uri = options.uri.replace('{applicationId}', application.applicationId)
                options.body = {
                    controlFlowId : application.controlFlowId
    ...
    this.submitApplication = function(application) {
        ...
        return new Promise(function(resolve, reject) {
            self.authenticate().then(function() {
                var options = self.getOptions("submit-application");
                options.pathname = options.pathname.replace('{applicationId}', application.applicationId)
                options.uri = options.uri.replace('{applicationId}', application.applicationId)
                options.body = {
                    controlFlowId : application.controlFlowId
    ...
    this.checkApplicationStatus = function(application) {
        var self = this;
        return new Promise(function(resolve, reject) {
            self.authenticate().then(function() {
                var options = self.getOptions("check-application-status");
                options.pathname = options.pathname.replace('{applicationId}', application.applicationId)
                options.uri = options.uri.replace('{applicationId}', application.applicationId)
                options.qs.controlFlowId = application.controlFlowId
    ...
    this.getOptions = function(service) {
        ...
        var qs = this.map.services[service].qs;
